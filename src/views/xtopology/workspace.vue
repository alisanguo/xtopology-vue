      ...
      <div v-if="tab===1">
        ...
        <!-- <div v-for="(item, index) in tools" :key="index">
          <div class="group" @click="item.expand=!item.expand">
            ...
            <span class="full">{{ item.group }}</span>
            <i v-if="item.expand" class="iconfont icon-angle-down"></i>
            <i v-if="!item.expand" class="iconfont icon-angle-right"></i>
          </div>
          <div class="buttons mb20" v-if="item.expand">
            <ng-container v-for="(button, i) in item.children"
              :key="i">
              <i
                v-if="!button.hidden"
...
import ImagesComponent from "@/components/users/ImagesComponent";
